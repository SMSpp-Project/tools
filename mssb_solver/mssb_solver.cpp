/*--------------------------------------------------------------------------*/
/*-------------------------- File mssb_solver.cpp --------------------------*/
/*--------------------------------------------------------------------------*/
/** @file
 *
 * This is a convenient tool for solving a MultiStageStochasticBlock.
 * The description of the MultiStageStochasticBlock must be given in a netCDF
 * file. This tool can be executed as follows:
 *
 *   ./mssb_solver [-s] [-e] [-m NUMBER] [-B FILE] [-S FILE] [-p PATH]
 *                 [-c PATH] < nc4-file >
 *
 * The only mandatory argument is the netCDF file containing the description
 * of the MultiStageStochasticBlock. This can be either a BlockFile or
 * a ProbFile. The BlockFile can contain any number of child groups, each one
 * describing a MultiStageStochasticBlock, each of which is then solved with
 ...
 * MultiStageStochasticBlock alongside the description of a BlockConfig and a
 * BlockSolverConfig for the MultiStageStochasticBlock; thus, every
 * MultiStageStochasticBlock is solved with these specified BlockConfig and
 * BlockSolverConfig.
 *
 ...
 * every MultiStageStochasticBlock; while the -S option specifies a
 * BlockSolverConfig file for every MultiStageStochasticBlock. If each of
 * these options is not provided when the given netCDF file is a BlockFile,
 * then default configurations are considered.
 *
...
{
 // mssb_solver has no tool-specific options
 return( false );
 }
 ...
 for( auto & problem : problems ) {  // for each problem descriptor:
  ...
  if( ! dynamic_cast< MultiStageStochasticBlock * >( block ) ) {
   std::cout << "Error: " << problem.first
	     << " not a MultiStageStochasticBlock" << std::endl;
   exit( 1 );
   }
...
int main( int argc , char ** argv )
{
 ...
 // MSSBSCfg.txt the BlockSolverConfig for the MultiStageStochasticBlock
 conf_prefix = "config/";
 default_bconf_name = "InnerBCfg.txt";
 default_sconf_name = "MSSBSCfg.txt";
