...
void print_help() {
 // http://docopt.org
 std::cout << "SMS++ unit commitment solver.\n"
           << std::endl
...
void process_args( int argc, char ** argv ) {
 ...
 if( argc < 2 ) {
  std::cout << "ucblock_solver: no input file\n"
            << "Try ucblock_solver --help' for more information.\n";
  exit( 1 );
 }
 /*--------------------------------------------------------------------------*/
 ...
 while( true ) {
  ...
  switch( opt ) {
   ...
   case '?':
   default:
    std::cout << "Try ucblock_solver --help' for more information.\n";
    exit( 1 );
  }
 }
